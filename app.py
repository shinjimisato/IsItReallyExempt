    ...
    with tabs[0]:
        ...
        if dashboard_data:
            ...
            with col2:
                st.metric(
                    "Avg Exempt Ratio",
                    format_percentage(summary.get('avg_exempt_ratio', 0), 3),
            ...
            with col3:
                st.metric(
                    "Highest Activity",
                    summary.get('highest_symbol', 'N/A'),
                    help="Symbol with most anomalies"
                )

            with col4:
                pattern_detected = "Yes" if summary.get('patterns_detected', 0) > 0 else "No"
                st.metric(
                    "Pattern Detected",
                    pattern_detected,
            ...
            if alerts:
                ...
                for alert in alerts[:10]:  # Top 10
                    alert_data.append({
                        ...
                        "Exempt Volume": format_number(alert['exempt_volume']),
        ...
        if selected_symbol and selected_symbol in data_dict:
            ...
            with st.expander("🗓️ Pattern Analysis", expanded=True):
                ...
                with col2:
                    ...
                    if dow_patterns:
                        ...
                        for day, count in sorted(dow_patterns.items(), key=lambda x: x[1], reverse=True)[:3]:
                    ...
                    if velocity:
                        ...
                        st.text(f"Avg daily change: {velocity.get('avg_pct_change', 0):.1f}%")
            ...
            with st.expander("📋 Detailed Anomaly List", expanded=False):
                ...
                if exempt_analysis and exempt_analysis.get('anomalies'):
                    ...
                    with col3:
                        percentiles = exempt_analysis.get('percentile_thresholds', {})
